...
int readlines(FILE *hamlet, char** buf, const char* filename, int num);
...
int main(const char* file1, const char* file2) {
    ...
    fseek(hamlet, 0, SEEK_END); // combine to "file size" function
    ...
    if ((nlines = readlines(hamlet, &buf, file1, num)) >= 0) {
...
//read lines:
int readlines(FILE *hamlet, char** buf, const char* filename, int num) { // rename
    ...
    *buf = (char*)calloc((buff.st_size + 1), sizeof(char));
    ...
    fclose(hamlet); // move it from here
